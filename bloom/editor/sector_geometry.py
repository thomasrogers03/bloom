...
class SectorGeometry:
    ...
    def add_2d_geometry(self, point_1: core.Point2, point_2: core.Point2, colour: core.Vec4):
        if not constants.PORTALS_DEBUGGING_ENABLED:
            segments = self._create_2d_segments(point_1, point_2, colour)
            segments.create(self._node_2d, dynamic=False)

    def add_2d_highlight_geometry(self, parent: core.NodePath, point_1: core.Point2, point_2: core.Point2):
        segments = self._create_2d_segments(point_1, point_2, core.Vec4(1, 1, 1, 1))
        highlight_display_node = segments.create(dynamic=False)
        highlight_display: core.NodePath = parent.attach_new_node(highlight_display_node)
    ...
    def _create_2d_segments(self, point_1: core.Point2, point_2: core.Point2, colour: core.Vec4):
        segments = core.LineSegs('2d')
        segments.set_thickness(2)
        segments.set_color(colour)
        segments.draw_to(core.Point3(point_1.x, point_1.y, -constants.REALLY_BIG_NUMBER))
        segments.draw_to(core.Point3(point_2.x, point_2.y, -constants.REALLY_BIG_NUMBER))
