...
class Segment:
    ...
    def project_point(self, point: core.Point2, ignore_on_line=False):
        direction = self.get_direction()
        ...
        offset = point - self.point_1
        portion = offset.dot(direction) / length_squared

        if not ignore_on_line and (portion < 0 or portion > 1):
            return None
