...
class ViewClipping:
    ...
    def __init__(
        ...
        if constants.PORTALS_DEBUGGING_ENABLED:
            ...
            self._inverse_transformation_matrix.invert_from(self._transformation_matrix)
    ...
    def _draw_debug_clip(self, colour, left: float, right: float):
        ...
        left_3d = self._inverse_transformation_matrix.xform_point_general(screen_left)
        left_3d.z = builder_pos.z

        screen_right = core.Point3(right, 0, depth)
        right_3d = self._inverse_transformation_matrix.xform_point_general(screen_right)
    ...
    def get_clipped_regions(self):
        ...
        if in_clip:
            regions.append(
                (self._clip_index_to_clip_range(left), WallBunch.ABSOLUTE_MAX_X)
            )
    ...
    def _do_clip(self, sector: EditorSector, depth=0):
        ...
        walls = {
            wall: ProjectedWall(wall, self._project_point) for wall in sector.walls
        }
        ...
        sorted_walls: typing.List[ProjectedWall] = sorted(
            walls.values(), key=self._wall_distance
        )
    ...
    def _increment_debug_colour(self):
        self._debug_colour_index = (self._debug_colour_index + 1) % len(
            self._BUNCH_COLOURS
        )
    ...
    def _debug_sector(self, sector: EditorSector, depth: int):
        if self._can_debug(depth):
            for editor_wall in sector.walls:
                ...
                offset = core.Vec3(offset_2d.x, offset_2d.y, 0) + core.Vec3(0, 0, -256)
                ...
                self._debug_wall_projection(
                    editor_wall, theta, position, core.Vec4(1, 0, 0, 1)
                )
                self._debug_wall_projection(
    ...
    def _can_debug(depth: int):
        return (
            constants.PORTALS_DEBUGGING_ENABLED
            and depth >= constants.PORTAL_DEBUG_DEPTH
        )
    ...
    def _debug_wall_projection(
        self,
        editor_wall: EditorWall,
        theta: float,
    ...
    def _make_debug_text(
        self, message: str, position: core.Point3, colour: core.Vec4, theta: float
    ):
    ...
    def _project_point(self, point: core.Point2) -> core.Point3:
        point_3d = core.Point3(
            point.x + self._sector_offset.x, point.y + self._sector_offset.y, 0
        )
    ...
    def _clip_range_to_clip_index(value: int):
        value = int(
            ((value - WallBunch.ABSOLUTE_MIN_X) / WallBunch.WIDTH_RANGE)
            * ViewClipping._CLIP_WIDTH
        )
        if value < 0:
            return 0
    ...
    def _wall_completely_invisible(wall: ProjectedWall):
        behind_camera = (
            wall.point_1.z >= WallBunch.ABSOLUTE_MAX_Z
            and wall.point_2.z >= WallBunch.ABSOLUTE_MAX_Z
        )
        too_far_left = (
            wall.point_1.x <= WallBunch.ABSOLUTE_MIN_X
            and wall.point_2.x <= WallBunch.ABSOLUTE_MIN_X
        )
        too_far_right = (
            wall.point_1.x >= WallBunch.ABSOLUTE_MAX_X
            and wall.point_2.x >= WallBunch.ABSOLUTE_MAX_X
        )
