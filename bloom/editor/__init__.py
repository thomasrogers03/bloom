...
def to_degrees(build_angle: int) -> float:
    return round((180 * build_angle) / 1024 + 90)
...
def to_build_sprite_repeat(sprite_repeat: float) -> int:
    return _valid_unsigned_byte(sprite_repeat * 4)
...
def to_build_panning_y(build_panning_y: float) -> int:
    return round(build_panning_y * 2) & 0xFF


def to_shade(build_shade: int):
    return 1 - (build_shade / 128)


def to_build_shade(shade: float):
    return _valid_signed_byte((1 - shade) * 128)
...
def _valid_signed_byte(value: float):
    value = round(value)
