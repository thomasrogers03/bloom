...
class TargetView:
    ...
    def _show_sources(
        self,
        target: map_objects.empty_object.EmptyObject
    ):
    ...
    def _show_targets(
        ...
        for target_index, target in enumerate(source.target_event_grouping.targets):
            self._draw_line(target_index, colour, next_colour, source.origin, target.origin)
            self._show_targets(target, next_colour)

    def _draw_line(self, index: int, colour: core.Vec4, next_colour: core.Vec4, point_1: core.Vec3, point_2: core.Vec3):
