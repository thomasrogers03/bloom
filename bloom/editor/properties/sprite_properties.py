...
class SpriteDialog:
    ...
    def __init__(
        ...
        self._tile_section = DirectGui.DirectFrame(
            parent=self._dialog, pos=core.Vec3(0.65, 0.07), frameColor=(0, 0, 0, 0)
        )
    ...
    def _save_changes(self):
        ...
        if target_special_value == "Next Level":
            ...
            self._sprite.target_event_grouping is not None
            and self._sprite.target_event_grouping.special_receiver_id is not None
        ):
        ...
        if source_special_value == "Level Start":
            ...
            self._sprite.source_event_grouping is not None
            and self._sprite.source_event_grouping.special_receiver_id is not None
        ):
    ...
    def _update_sprite_tile(self, picnum: int):
        ...
        texture = self._tile_viewer.tile_manager.get_tile(
            self._current_picnum, self._current_palette
        )

        frame_size = gui.size_inside_square_for_texture(
            texture, self._TILE_FRAME_SIZE_INNER
        )
