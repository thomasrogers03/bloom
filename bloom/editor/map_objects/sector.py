...
class EditorSector(empty_object.EmptyObject):
    ...
    def split(
        ...
        for point in points:
            ...
            new_wall = self.add_wall(new_blood_wall)
        ...
        segments = zip(otherside_walls, new_walls,
    ...
    def closest_object_intersecting_line(self, point: core.Point3, direction: core.Vec3):
        ...
        part = self.part_for_direction(normalized_direction)
        if part == self.FLOOR_PART:
            hit = self.floor_plane.intersect_line(point, normalized_direction)
        else:
            hit = self.ceiling_plane.intersect_line(
                point, normalized_direction)
        if hit is not None and self._point_in_sector(core.Point2(hit.x, hit.y)):
            closest_object = self
            closest_part = part
            closest_hit = hit
    ...
    def _hit_3d_and_squared_distance_from_hit_2d(
        ...
        above_floor = hit.z <= self.floor_z_at_point(intersection_2d)
        below_ceiling = hit.z >= self.ceiling_z_at_point(intersection_2d)
