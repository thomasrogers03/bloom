...
_SPRITE_CATEGORIES_TYPE = typing.Dict[str, typing.List[type_descriptor.Descriptor]]
...
def _load_sprite_descriptors():
    ...
    for sprite_type, descriptor in sprite_types.items():
        result[sprite_type] = type_descriptor.Descriptor(sprite_type, descriptor)
    return result
...
def _categorized_sprites(
    ...
    result: _SPRITE_CATEGORIES_TYPE = {
        category_name: [] for category_name in category_names
    }
...
categorized_sprites = _categorized_sprites(
    sprite_category_descriptors.keys(), sprite_types
)
