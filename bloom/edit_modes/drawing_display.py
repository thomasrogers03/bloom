...
class DrawingDisplay:
    def __init__(self, camera_collection: cameras.Cameras):
        ...
        self._grid = grid_display.GridDisplay(
            self._camera_collection, self._camera_collection.scene
        )
        self._grid.hide()
    ...
    def update(
        ...
        for point_1, point_2 in walls:
            point_3d_1 = core.Point3(
                point_1.x, point_1.y, self._floor_z_at_point(point_1)
            )
            segments.draw_to(point_3d_1)

            point_3d = core.Point3(
                point_1.x, point_1.y, self._ceiling_z_at_point(point_1)
            )
            segments.draw_to(point_3d)

            point_3d = core.Point3(
                point_2.x, point_2.y, self._ceiling_z_at_point(point_2)
            )
            segments.draw_to(point_3d)

            point_3d = core.Point3(
                point_2.x, point_2.y, self._floor_z_at_point(point_2)
            )
            segments.draw_to(point_3d)
