...
class EditMode(navigation_mode_2d.EditMode):
    def __init__(self, editor_dialogs: dialogs.Dialogs, *args, **kwargs):
        ...
        self._drawing_mode = drawing_mode_2d.EditMode(*args, **kwargs)
    ...
    def enter_mode(self, state: dict):
        ...
        self._context_menu.add_command(
            "Flip Sectors Vertically", self._flip_sectors_vertically
        )
        self._context_menu.add_command(
            "Flip Sectors Horizontally", self._flip_sectors_horizontally
        )
    ...
    def _copy_sectors(self):
        ...
        self._sector_prefab = operations.sector_copy.SectorCopy(
            list(sectors), self._editor.sectors
        )
    ...
    def _start_drawing_new_sector(self):
        ...
        self._drawing_mode.start_drawing(None, target, True)
    ...
    def _select_from_marquee(self):
        ...
        finder = find_in_marquee.FindInMarquee(
            self._editor.sectors.sectors, start.xy, end.xy
        )
    ...
    def _mouse_collision_tests(self):
        with self._edit_mode_selector.track_performance_stats(
            "mouse_collision_tests_2d"
        ):
    ...
    def _make_highlight_finder_callback(self):
        ...
        return highlight_finder_2d.HighlightFinder2D(
            self._editor, source.xy, self._camera_scale()
        ).find_highlight
